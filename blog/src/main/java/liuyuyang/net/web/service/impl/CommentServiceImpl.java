...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public void add(Comment comment) throws Exception {
        ...
        // 判断是否还有上一条评论        Comment prev_comment = null;
    ...
    public Comment get(Integer id) {
        ...
        // 获取所有相关评论        QueryWrapper<Comment> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("article_id", data.getArticleId());
        List<Comment> list = commentMapper.selectList(queryWrapper);

        // 构建评论树        data.setChildren(buildCommentTree(list, data.getId()));
    ...
    public List<Comment> list(CommentFilterVo filterVo) {
        ...
        for (Comment data : list) {
            // 绑定对应的数据            Article article = articleMapper.selectById(data.getArticleId());
